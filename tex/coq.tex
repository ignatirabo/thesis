...
% TODO: qué es una goal?
Por ejemplo, supongamos que queremos probar el siguiente teorema.
\begin{lstlisting}
Definition le_S (n : nat) : n <= S n.
\end{lstlisting}
Al entrar a alguno de los editores de textos compatibles con Coq (Emacs, Visual Studio Code o CoqIDE) cargamos el teorema y Coq entrará al modo interactivo, en el cual nos dirá el estado actual de la prueba. En este caso como no hicimos nada hasta el momento, tenemos una hipótesis \lstinline{n : \nat} y una única \textit{goal}: \lstinline{n <= S n}. Entonces aplicamos inducción en \lstinline{n} tenemos dos submetas. Y así Coq nos asiste en el proceso, mostrándonos nuestras hipótesis y metas actuales, como podemos observar en \ref{fig:ui}.
...
A continuación explicaremos los conceptos de \textit{táctica} y de que manera se utiliza Coq.
...
Las tácticas se utilizan para probar teoremas. Hemos utilizado \lstinline{induction} y \lstinline{apply} para el ejemplo de la figura \ref{fig:ui} pero ahora hablaremos de varias de ellas y cómo el meta-lenguaje Ltac (ya en Coq por defecto) nos permite escribirlas.
...
Como se documenta en ... hay cientos de tácticas. Hablaremos de algunas solo para que el lector pueda tener un concepto de cómo puede ser una táctica, y luego diseñaremos alguna táctica muy simple.
\begin{itemize}
  \item \lstinline{simpl}: esta táctica aplica a cualquier meta. Intenta reducir un término a algo legible y no normalizarlo completamente.
  \item \lstinline{intros}: introduce todas las hipotesis que todavía estén en la meta. 
  \item \lstinline{rewrite}: reescribe la meta utilizando alguna igualdad previamente provada o alguna hipótesis.
  \item \lstinline{destruct}: toma una variable inductiva o co-inductiva y genera submetas para cada uno de sus constructores.
  \item \lstinline{induction}: hace lo mismo que \lstinline{destruct} con la diferencia de que genera hipótesis inductivas.
\end{itemize}
Estas tácticas son algunas de las más utilizadas.
