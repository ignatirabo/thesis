...
Para ejecutar \lift\ con \lstinline{bind} ejecutamos el siguiente comando:
\begin{lstlisting}
Definition li_bind : m:{T : TyTree & to_ty T} :=
  ltac:(mrun (lift' bind m)).
\end{lstlisting}
\pause
Como la signatura muestra, \lstinline{li_bind} es un $\Sigma$-type que contiene al resultado.
\vskip \baselineskip

\pause
Con los siguientes comandos podemos extraer la signatura y la función resultante:
\begin{lstlisting}
...
Respuesta de \lstinline{Eval cbn in to_ty (mprojT1 li_bind)}:
\begin{lstlisting}
\forall A B : \forall l : list nat, l <> nil -> Type,
  (\forall (l : list nat) (p : l <> nil), M (A l p)) ->
  (\forall (l : list nat) (p : l <> nil), A l p -> M (B l p)) ->
  \forall (l : list nat) (p : l <> nil), M (B l p)
\end{lstlisting}
\vskip \baselineskip

\pause
Respuesta de \lstinline{Eval cbn in mprojT2 li_bind}:
\begin{lstlisting}
fun A B x f l p => bind (A l p) (B l p) (x l p) (f l p)
\end{lstlisting}
