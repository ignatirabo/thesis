...
En este capitulo introduciremos las características más relevantes del asistente de pruebas interactivo Coq. El objetivo de este capitulo es introducir todos los conceptos que utilizaremos más adelante.
...
En 1991, Coquand creo una derivación del trabajo anterior, \emph{Calculus of Inductive Constructions} \cite{CIC}. Esta teoría comenzó a ser desarrollada y el producto final es lo que hoy llamamos Coq.
...
\begin{description}
    ...
    \item[Vernacular] Este lenguaje contiene los comandos básicos de Coq con los que nos comunicamos con el intérprete.
...
Como podemos observar bool es un tipo inductivo, especificado por el comando \lstinline{Inductive} de Vernacular, con dos constructores \lstinline{true} y \lstinline{false}. De por sí, el tipo \lstinline{bool} no posee un significado hasta que nosotros lo proveamos de uno. Podemos ahora intentar definir algunos operadores booleanos.
\begin{lstlisting}
...
Intuitivamente, los $\Sigma$\textit{-types} son duplas donde el argumento de la derecha es dependiente del de la izquierda. También podemos llamarlos pares dependientes. A continuación, la definición.
